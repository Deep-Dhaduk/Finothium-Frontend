...
import { clientIcon, rupeeSymbol } from 'src/varibles/icons'
...
const AccountReport = () => {
    ...
    const fetchChartData = () => {
        return reportData.reduce(
            (acc, item) => {
                acc.label.push(item.clientName);
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterClientWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Client"} clientReportData={true} expanded={expanded} handleExpandAll={handleExpandAll} reportData={reportData} setReportData={setReportData} filterDropdownData={filterDropdownData} fileName={"ClientData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <><Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        ...
                        >
                            <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                <Icon icon={clientIcon} fontSize={22} />
                            </Box>
                            <PageHeader title={<Typography variant='h6' >{item.clientName}</Typography>}></PageHeader>
