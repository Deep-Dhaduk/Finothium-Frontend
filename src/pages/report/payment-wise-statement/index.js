...
import { paymentTypeIcon, rupeeSymbol } from 'src/varibles/icons'
...
const AccountReport = () => {
    ...
    const fetchChartData = () => {
        return reportData.reduce(
            (acc, item) => {
                acc.label.push(item.PaymentName);
    ...
    const fetchData = async () => {
        try {
            ...
            setShowSpinner(true);
            const response = await filterPaymentWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Payment Type"} reportData={reportData} setReportData={setReportData} paymentReportData={true} expanded={expanded} handleExpandAll={handleExpandAll} filterDropdownData={filterDropdownData} fileName={"PaymentTypeData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <> <Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        ...
                        >
                            <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                <Icon icon={paymentTypeIcon} fontSize={22} />
                            </Box>
                            <PageHeader title={<Typography variant='h6' >{item.PaymentName}</Typography>}></PageHeader>
