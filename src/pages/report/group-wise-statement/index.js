...
const defaultColumns = [
    ...
    },
    {
        ...
        field: 'receiveAmount',
        headerName: 'Receive',
        renderCell: ({ row }) => <Typography variant='paragraph' sx={{ color: 'info.main', display: 'flex', alignItems: 'center' }}>
            {row.receiveAmount !== null && <Icon icon={rupeeSymbol} fontSize={16} />}{row.receiveAmount}</Typography>
    },
    {
        flex: 0.25,
        minWidth: 80,
        field: 'paidAmount',
        headerName: 'Paid',
        renderCell: ({ row }) => <Typography variant='paragraph' sx={{ color: 'warning.main', display: 'flex', alignItems: 'center' }}>
            {row.paidAmount !== null && <Icon icon={rupeeSymbol} fontSize={16} />}{row.paidAmount}</Typography>
    },
    {
        flex: 0.25,
        minWidth: 250,
...
const AccountReport = () => {
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterGroupWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Account Group"} groupReportData={true} expanded={expanded} handleExpandAll={handleExpandAll} reportData={reportData} setReportData={setReportData} filterDropdownData={filterDropdownData} advanceFilterConfig={advanceFilterConfig} fileName={"AccountGroupData"} />
            </Grid>
            ...
            {reportData.map((item, index) => {
                ...
                return <Grid sx={{ mb: 5 }}>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <><Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        <AccordionSummary
                            ...
                            <PageHeader title={<Typography variant='h6' >{item.GroupName}</Typography>}></PageHeader>
