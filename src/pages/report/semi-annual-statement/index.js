...
import { rupeeSymbol, semiAnnualIcon } from 'src/varibles/icons';
...
const SemiAnnualReport = () => {
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterSemiAnnualWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Semi Annual"} semiAnnualData={true} expanded={expanded} handleExpandAll={handleExpandAll} reportData={reportData} setReportData={setReportData} filterDropdownData={filterDropdownData} fileName={"SemiannualData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <> <Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        <AccordionSummary
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                <Icon icon={semiAnnualIcon} fontSize={22} />
                            </Box>
