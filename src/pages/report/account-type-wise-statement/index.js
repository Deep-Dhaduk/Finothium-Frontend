...
import { accountTypeIcon, rupeeSymbol } from 'src/varibles/icons'
...
const AccountTypeReport = () => {
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterAccountTypeWiseData(filterData)
    ...
    const fetchChartData = () => {
        return reportData.reduce(
            (acc, item) => {
                acc.label.push(item.accountTypeName);
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            ...
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Account Type"} reportData={reportData} setReportData={setReportData} accountTypeReportData={true} handleExpandAll={handleExpandAll} expanded={expanded} filterDropdownData={filterDropdownData} fileName={"AccountTypeData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>

            <ChartComponant chartData={chartData} />

            {reportData.map((item, index) => {
                const id = item.accountTypeId
                        ...
                        <> <Accordion defaultExpanded={[id]} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                            <AccordionSummary
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                    <Icon icon={accountTypeIcon} fontSize={22} />
                                </Box>
                                <PageHeader title={<Typography variant='h6' >{item.accountTypeName}</Typography>}></PageHeader>
