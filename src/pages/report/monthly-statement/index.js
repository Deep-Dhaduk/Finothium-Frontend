...
import { CustomNoRowsOverlay, AppTooltip, cardTextColor, formatDate, formatMonth, getAllFilterData } from 'src/allFunction/commonFunction';
...
import { monthlyIcon, rupeeSymbol } from 'src/varibles/icons';
...
const MonthlyReport = () => {
    ...
    const fetchChartData = () => {
        return reportData.reduce(
            (acc, item) => {
                acc.label.push(formatMonth(item.fiscalName));
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterMonthlyWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Month"} monthlyData={true} expanded={expanded} handleExpandAll={handleExpandAll} reportData={reportData} setReportData={setReportData} filterDropdownData={filterDropdownData} fileName={"MonthlyData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <><Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        <AccordionSummary
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                <Icon icon={monthlyIcon} fontSize={22} />
                            </Box>
                            <PageHeader title={<Typography variant='h6' >{formatMonth(item.fiscalName)}</Typography>}></PageHeader>
