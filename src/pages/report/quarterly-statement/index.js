...
import { quarterlyIcon, rupeeSymbol } from 'src/varibles/icons';
...
const QuarterlyReport = () => {
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterQuarterlyWiseData(filterData)
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            ...
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Quarter"} quarterlyData={true} expanded={expanded} handleExpandAll={handleExpandAll} reportData={reportData} setReportData={setReportData} filterDropdownData={filterDropdownData} fileName={"QuartelyData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                    ...
                    {reportViewSetting.summary || reportViewSetting.detail ? <><Accordion defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                        <AccordionSummary
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                <Icon icon={quarterlyIcon} fontSize={22} />
                            </Box>
