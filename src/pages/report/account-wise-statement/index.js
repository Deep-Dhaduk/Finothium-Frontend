...
import { accountIcon, rupeeSymbol } from 'src/varibles/icons'
...
const AccountReport = () => {
    ...
    const fetchData = async () => {
        try {
            setShowSpinner(true);
            const response = await filterAccountWiseData(filterData)
    ...
    const fetchChartData = () => {
        return reportData.reduce(
            (acc, item) => {
                acc.label.push(item.accountName);
    ...
    return (
        <AppContext.Provider value={{ allIds, setAllIds }} >
            <Grid>
                <TableHeader value={value} handleFilter={handleFilter} title={"Account"} reportData={reportData} setReportData={setReportData} accountReportData={true} expanded={expanded} handleExpandAll={handleExpandAll} filterDropdownData={filterDropdownData} fileName={"AccountData"} advanceFilterConfig={advanceFilterConfig} />
            </Grid>
                ...
                reportData.map((item, index) => {
                    ...
                    return <Grid sx={{ mb: 5 }}>
                        ...
                        {reportViewSetting.summary || reportViewSetting.detail ?
                            <><Accordion key={index} defaultExpanded={defaultOpen} expanded={expanded.includes(index)} onChange={() => handleAccordionChange(index)}>
                                <AccordionSummary
                                    expandIcon={<Icon icon='mdi:chevron-down' fontSize={20} />}
                                    id={id} >
                                    <Box sx={{ display: 'flex', alignItems: 'center', mr: '10px' }}>
                                        <Icon icon={accountIcon} fontSize={22} />
                                    </Box>
                                    <PageHeader title={<Typography variant='h6' >{item.accountName}</Typography>}></PageHeader>
