...
import { account_Type } from 'src/varibles/constant'
...
const AccountType = () => {
  ...
  const accountData = store.data.filter(item => item.type === 'Account Type')
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      if (!response.success) {
        ...
        setEditDialogOpen(false)
        showSuccessToast('Account Type Updated Successfully.')
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (!response.success) {
          ...
          showSuccessToast('Account Type Deleted Successfully.')
        }
      }
    } catch (error) {
      console.error('An error occurred:', error)
      showErrorToast('An error occurred while deleting the account type.')
    } finally {
      setShowSpinner(false)
    }
  }
        ...
        <Grid container spacing={6}>
          ...
          <Grid item xs={12}>
            <Card>
              ...
              />
              <CommonMenu rows={commonData} columns={columns} />
            </Card>
          </Grid>
        </Grid>
