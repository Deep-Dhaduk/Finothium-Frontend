...
import { category, payment, paymentType, subCategory } from 'src/varibles/constant'
...
const PaymentTransaction = () => {
  ...
  const defaultColumns = [
      ...
      renderCell: ({ row }) => (
        <Grid container>
          <Grid item xs={11} sm={11}>
            <Typography variant='paragraph' sx={{ color: 'warning.main', display: 'flex', alignItems: 'center' }}>
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Typography variant='paragraph' className='text-color'>
          {row.description}
        </Typography>
      )
    },
  ...
  const handleDelete = async id => {
    try {
      ...
      showErrorToast('An error occurred while deleting the payment.')
    } finally {
      setShowSpinner(false)
    }
  }
  ...
  const fetchActiveCategory = async () => {
    const response = await fetchActiveClientData({ type: category })
          ...
          <Dialog maxWidth='lg' fullWidth onClose={handleDialogToggle} open={editDialogOpen}>
            <DialogTitle sx={{ pt: 4, textAlign: 'center' }}>
              <Typography variant='h6' component='span' sx={{ mb: 2 }}>
                {isEdit ? 'Edit' : 'Add'} Payment Transaction
              </Typography>
            </DialogTitle>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={6}>
                        <FormControl fullWidth sx={{ mb: 4 }}>
                          <Autocomplete
                            fullWidth
                            value={activeAccount.find(item => item.account_id === editValue.accountName) || null}
